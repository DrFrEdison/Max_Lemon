...
dir( pattern = "Rsource" )
source.file <- print("Rsource_Max_Lemon_mtx_mop_val_V01.R")
...
dt$model.raw <- dt$model.raw[ !is.na(dt$model.raw$TA) , ]
dt$model.raw$TA <- dt$model.raw$TA * 10
...
matplot(dt$para$wl[[1]]
        , t( dt$SL$spc[ grep("SK", dt$SL$data$Probe) , ])
...
dt$para.pls$wlr <- wlr_function(210:320, 210:320, 5); nrow(dt$para.pls$wlr)
dt$para.pls$wlm <- wlr_function_multi(210:320, 210:320, 10); nrow(dt$para.pls$wlm)
